...
public class Main {
    public static void main(String[] args) {
        ...
        double totalChico = 0;
        for (Inscripcion i : inscripcionesChico) {
            totalChico += i.getTotal();
        }
        System.out.println("\nTotal categoria chico: " + totalChico);

        double totalMedio = 0;
        for (Inscripcion i : inscripcionesMedio) {
            totalMedio += i.getTotal();
        }
        System.out.println("\nTotal categoria medio: " + totalMedio);

        double totalAvanzado = 0;
        for (Inscripcion i : inscripcionesAvanzado) {
            totalAvanzado += i.getTotal();
        }
        System.out.println("\nTotal categoria avanzado: " + totalAvanzado);

        System.out.println("\nTotal todas las categorias " + (totalChico + totalMedio + totalAvanzado));
    }
}