...
public class UserController {
  ...
  public ResponseEntity<FollowersDto> getFollowers(@PathVariable
  ...
  public ResponseEntity<LastPostsDto> lastPosts(@PathVariable
                                                  @Min(message = "El id debe ser mayor a 0.", value = 1)
                                                  int userId,
  ...
  public ResponseEntity<UserWFollowedDto> followedList(@PathVariable
                                                         @Min(message = "El id debe ser mayor a 0.", value = 1)
                                                         int userId, @RequestParam(required = false) String order){
    return ResponseEntity.ok(this.userService.getFollowed(userId, order));
  }

  @GetMapping("/users/{id}/followers/list")
  public ResponseEntity<UserWFollowerListDto> getFollowersById(@PathVariable
                                                                 @Min(message = "El id debe ser mayor a 0.", value = 1)
                                                                 int id, @RequestParam(required = false) String order) {
