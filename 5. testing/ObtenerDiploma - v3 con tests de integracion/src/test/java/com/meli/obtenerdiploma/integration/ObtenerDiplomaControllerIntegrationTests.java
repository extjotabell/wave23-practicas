...
public class ObtenerDiplomaControllerIntegrationTests {
    ...
    public void analyzeScoresTest() throws Exception {
        ...
        MvcResult result = this.mockMvc.perform(get("/analyzeScores/{studentId}", 1))
                                .andDo(print())
                                .andExpect(status().isOk())
                                .andExpect(content().contentType("application/json"))
                                .andExpect(jsonPath("$.message").value("El alumno Marcos ha obtenido un promedio de 8.00. Puedes mejorar."))
                                .andReturn();
    ...
    public void analyzeScoresExceptionTest() throws Exception {
        this.mockMvc.perform(get("/analyzeScores/{studentId}", 1000))
                .andDo(print())
                .andExpect(status().isNotFound())
                .andExpect(content().contentType("application/json"))
                .andExpect(jsonPath("$.description").value("El alumno con Id 1000 no se encuetra registrado."));
    }
