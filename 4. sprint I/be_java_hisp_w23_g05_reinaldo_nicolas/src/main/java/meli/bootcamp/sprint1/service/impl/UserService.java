...
public class UserService implements IUserService {
  ...
  @Override
  public BaseResponseDto addPromoPost(NewPostDto newPost) {
    User user = this.repository.findUserById(newPost.getUser_id());
    if (user == null) {
      throw new BadRequestException("User not found");
    }
  ...
  @Override
  public PromoPostDto getPromoPost(int userId) {
    User user = this.repository.findUserById(userId);
    if (user == null) {
      throw new BadRequestException("User not found");
    }

    return new PromoPostDto(user.getId(),
            user.getName(),
            user.getPosts().stream().filter(Post::isHas_promo).toList().size());
  }

  @Override
  public CategoryCountDto getCountCategoryUser(int categoryId, int userId) {
    User user = this.repository.findUserById(userId);
    ...
    Category category = this.repository.findCategoryById(categoryId);
    if (category == null) {
      throw new BadRequestException("Category not found");
    }
    ...
    return new CategoryCountDto(
            ...
            user.getPosts().stream()
                    .filter(p -> p.getCategory().getDescription().equals(category.getDescription()))
                    .toList().size());
  }
}