...
public class SocialMeliController {
    ...
    public ResponseEntity<?> savePostPromo(@RequestBody PostPromoDTO postPromo) {
        return new ResponseEntity<>(socialMediaService.savePost(postPromo), HttpStatus.OK);
    }

    @GetMapping("/products/promo-post/count")
    public ResponseEntity<?> getProductsInPromoByUserId(@RequestParam Integer user_id) {
        return new ResponseEntity<>(socialMediaService.getProductsInPromoByUserId(user_id), HttpStatus.OK);
    }

    @PutMapping("users/{userId}/modify-promo/{promoPostId}")
    public ResponseEntity<?> modifyDiscountPromoPost(@PathVariable Integer userId,
                                                     @PathVariable Integer promoPostId,
                                                     @RequestParam Double newDiscount) {
        return new ResponseEntity<>(socialMediaService.modifyDiscountPromoPost(userId, promoPostId, newDiscount), HttpStatus.OK);
    }
}
