...
public class SocialMediaServiceImpl implements ISocialMediaService {
    ...
    public SimpleMessageDTO savePostWithPromo(PostDTO postPromo){
        Verifications.validatePostHasPromo(postPromo);
        return savePost(postPromo);
    }

    public PromoPostsCountDTO getPostsWithPromoFromUserId(int userId){
        User user = socialMediaRepository.findUser(userId);

        verifyUserExist(user);

        return new PromoPostsCountDTO(
                user.getId(),
                user.getName(),
                user.getPosts().stream().filter(Post::isHasPromo).toList().size()
        );
    }

    public SimpleMessageDTO deletePost(int postId, int userId){
        Verifications.verifyUserExist(socialMediaRepository.findUser(userId));
        Post post = socialMediaRepository.findPost(postId, userId);
        Verifications.validatePostExists(post);
        socialMediaRepository.deletePost(postId, userId);
        return new SimpleMessageDTO("Se ha eliminado el Post id: " + postId);
    }
}
