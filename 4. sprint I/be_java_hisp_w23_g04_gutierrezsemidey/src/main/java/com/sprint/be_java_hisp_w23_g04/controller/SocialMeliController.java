...
public class SocialMeliController {
    ...
    public ResponseEntity<?> savePostWithPromo(@RequestBody PostDTO postPromo){
        return new ResponseEntity<>(socialMediaService.savePostWithPromo(postPromo), HttpStatus.OK);
    }

    @GetMapping("/products/promo-post/count")
    public ResponseEntity<?> savePostWithPromo(@RequestParam(name = "user_id") int userId){
        return new ResponseEntity<>(socialMediaService.getPostsWithPromoFromUserId(userId), HttpStatus.OK);
    }

    @DeleteMapping("/user/{userId}/posts/{postId}/delete")
    public ResponseEntity<?> deletePost(@PathVariable int postId, @PathVariable int userId){
        return new ResponseEntity<>(socialMediaService.deletePost(postId, userId), HttpStatus.OK);
    }
}
