...
public class UserMapper {
    ...
    public static User mapUser(DBUserDTO userDto) {
        ...
        List<Post> posts = postsDto.stream().map(p -> new Post(p.getPostId(), p.getDate(), mapProduct(p.getProduct()), p.getCategory(), p.getPrice(), p.isHasPromo(), p.getDiscount())).collect(Collectors.toList());
    ...
    public static Post mapPost(PostDTO post, int postId) {
        return new Post(postId, post.getDate(), mapProduct(post.getProduct()), post.getCategory(), post.getPrice(), post.isHasPromo(), post.getDiscount());
    }
