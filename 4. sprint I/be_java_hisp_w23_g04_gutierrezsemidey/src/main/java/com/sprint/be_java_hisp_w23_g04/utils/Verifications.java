...
public class Verifications {
    ...
    public static void validatePostHasPromo(PostDTO post){
        if(!post.isHasPromo()){
            throw new BadRequestException("El post que se está intentando crear debe tener promociones aplicadas." +
                    " Si está intentando hacer un post sin promociones, utilice /products/post ");
        }
        if(post.getDiscount() <= 0.0){
            throw new BadRequestException("El post que se está intentando crear debe tener un valor válido de descuento." +
                    " Si está intentando hacer un post sin promociones, utilice /products/post ");
        }
    }

    public static void validatePostExists(Post post){
        if(post == null){
            throw new NotFoundException("El usuario indicado no posee un post con el id indicado");
        }
    }
}
