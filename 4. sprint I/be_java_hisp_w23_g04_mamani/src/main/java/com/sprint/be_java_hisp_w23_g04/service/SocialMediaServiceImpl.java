...
public class SocialMediaServiceImpl implements ISocialMediaService {
    ...
    public SimpleMessageDTO savePromoPost(PromoPostDTO post) {
        ...
        int postId = socialMediaRepository.getNextPostId(user);
        user.getPosts().add(UserMapper.mapPromoPost(post, postId));
        socialMediaRepository.savePost(user);

        return new SimpleMessageDTO("El promo post para el user: " + user.getId() + " se guardó exitosamente");
    }
    ...
    public PromoPostCountDTO getPromoPostCount(int userId) {
        ...
        return new PromoPostCountDTO(
                user.getId(),
                user.getName(),
                user.getPosts().stream().filter(post -> post instanceof PromoPost)
                        .map(post -> (PromoPost) post).toList().size()
        );
    }

    @Override
    public FollowedCountDTO getFollowedCount(int userId) {
        User user = socialMediaRepository.findUser(userId);
        verifyUserExist(user);

        return new FollowedCountDTO(
                user.getId(),
                user.getName(),
