...
public class UserMapper {
    ...
    public static UserDTO mapUser(User user) {
        List<PostResponseDTO> postResponseDTOS = user.getPosts().stream()
                .map(p -> PostMapper.PostRequestDTOMapper(user.getId(), p)).toList();
    ...
    public static PromoPost mapPromoPost(PromoPostDTO post, int postId) {
        return new PromoPost(postId, post.getDate(), mapProduct(post.getProduct()),
