...
public class Mapper {
    ...
    public Post mapPostPromoDTOToPostPromo(PostPromoDTO postPromoDTO){
        return new Post(postPromoDTO.getId(),postPromoDTO.getUserId(), postPromoDTO.getDate(),mapProductDTOToProduct(postPromoDTO.getProduct()),postPromoDTO.getCategory(),postPromoDTO.getPrice(), postPromoDTO.isPromo(), postPromoDTO.getDiscount());
    }

    public PostPromoDTO mapPostPromoToPostPromoDTO(Post post){
        return new PostPromoDTO(post.getId(),post.getUserId(), post.getDate(),mapProductToProductDTO(post.getProduct()),post.getCategory(),post.getPrice(), post.isPromo(), post.getDiscount());
    }
