...
public class ProductController {
    ...
    public ResponseEntity<?> countPromoProduct(@RequestParam int user_id){
        return new ResponseEntity<>(productService.countPromoProduct(user_id),HttpStatus.OK);
    }

    @GetMapping("price-filter/{min}/{max}")
    public ResponseEntity<?> filterPriceProduct(@PathVariable double min,@PathVariable double max){
        return new ResponseEntity<>(productService.filterPriceProdutc(min,max),HttpStatus.OK);
    }
}
