...
public class ProductServiceImpl implements ProductService{
    ...
    public PromoPostResponseDTO postProduct(PromoPostRequestDTO promoPostRequestDTO){
        Seller seller = getSeller(promoPostRequestDTO.getUserId());
        validateProduct(promoPostRequestDTO.getProduct().getProductId());

        Post post = createPostWithPromo(promoPostRequestDTO, seller);
        return PostMapper.toPromoPostResponseDTO(post);
    }
    ...
    private Post createPostWithPromo(PromoPostRequestDTO promoPostRequestDTO, Seller seller) {
        Long id = productRepository.getNextId();
        Post post = PostMapper.toPostPromo(promoPostRequestDTO, seller, id);
        productRepository.create(post);
        seller.getPosts().put(id, post);
        return post;
    }
    ...
    @Override
    public PromoPostCountResponseDTO getPromoPostCount(Long userId) {
        Seller seller = getSeller(userId);
        return DTOMapper.mapToPromoPostCountDTO(seller);
    }
}